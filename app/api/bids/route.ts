...
/* ============================================================
   GET — Ranking + current bids + count_only mode
============================================================ */
export async function GET(req: Request) {
  try {
    ...
    if (!auctionId) {
      return NextResponse.json({ success: false, error: "auction_id required" }, { status: 400 });
    }

    // Return only total supplier count
    if (countOnly) {
    ...
    if (!supplierId) {
      return NextResponse.json(
        { success: false, error: "supplier_id required when full payload is needed" },
        { status: 400 }
      );
    }
    ...
    if (!auction) return NextResponse.json({ success: false, error: "Auction not found" });
    ...
    for (const it of items || []) qtyByItem[it.id] = Number(it.qty || 1);
...
export async function POST(req: Request) {
  try {
    ...
    if (auction.auction_type === "sealed_reverse") {
      ...
      if (count && count > 0)
        return NextResponse.json({ success: false, error: "Sealed bid already submitted" }, { status: 400 });
    }
